...
export function App() {
  ...
  const endRecording = () => {
    ...
    setFrozenJsonl(recording.toJsonl()); setRecordingActive(false);
  };
  ...
  const startRecording = () => {
    if (!connection || !note.trim()) { setStatus("Connect and enter a non-empty vehicle-state note before recording."); return; }
    ...
    setTranscript([]); setFrozenJsonl(undefined); setRecordingActive(true); setStatus("Recording started.");
  };
  const send = async () => {
    if (!session.current) return;
    setPending(true);
