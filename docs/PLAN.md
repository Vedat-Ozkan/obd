...
| ~~T0.3~~ ✅ 2026-09-23 | `obd-core`: `Transport` interface, `ReplayTransport` over the recording format, `ElmLineReader` (accumulate until `>`, strip echo, split lines, classify `NO DATA` / `SEARCHING...` / errors). | M | Unit tests against T0.2 recordings; every response class in `docs/ELM327.md` has a test. ICE recording replay remains hardware-only until T0.8 exports the Chrysler and Elantra baselines. |
