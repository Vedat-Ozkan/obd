...
| T0.8 | App: Expo dev build (EAS), `react-native-ble-plx` via config plugin, scan and connect to the Veepeak, `BleTransport` implementing `Transport` with MTU chunking and notify reassembly, a debug console (type a command, see the reply), and in-app recording export in the standard format. Runs right after T0.3 (it needs only `Transport` and `ElmLineReader`). | L | On a phone connected to the Equinox EV: `0100` succeeds from the console, one recording is exported and replays green. Start the EAS build early; it is mostly waiting. |
...
Goal: a used-EV battery health check a buyer can show a seller, verified first on GM Ultium (Equinox EV, then other Ultium models via their owners), a battery report for the owner's own car that they would trust more than the dash, a base layer for any other EV labeled by signal tier, and an opt-in LLM summary and assistant that never states a number the data does not contain. Built on verified signals and honest about unverified ones.
